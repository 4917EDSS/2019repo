...
void DriveWithJoystickCmd::Execute() {
  std::shared_ptr<frc::Joystick> driverJoystick = Robot::m_oi.getDriverController();

  double rightStick = driverJoystick->GetZ();
  double leftStick = driverJoystick->GetY();

  Robot::drivetrainSub.drive(leftStick, rightStick);
  std::cout << "Drive: " << leftStick << " | " << rightStick << std::endl;
}
