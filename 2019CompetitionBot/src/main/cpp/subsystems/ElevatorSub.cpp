...
void ElevatorSub::setElevatorHeight(int mode, double maxPower, double targetHeightMm)
{
  ...
  if ((mode != elevatorControlMode) || (maxPower != elevatorMaxPower) || (targetHeightMm != elevatorTargetHeightMm))
  {
    ...
    logger.send(logger.ELEVATOR, "ESH: change detected! Max power: %f \n", elevatorMaxPower);
    elevatorNewStateParameters = false;

    // Check input parameters
    if (fabs(maxPower) > 1.0)
    {
      logger.send(logger.ELEVATOR, "ESH: ERROR!! Max power = %f \n", maxPower);
      return;
    }
    if ((mode != ELEVATOR_MODE_MANUAL) && (targetHeightMm < ELEVATOR_MIN_HEIGHT_MM) || (targetHeightMm > ELEVATOR_MAX_HEIGHT_MM))
    {
      logger.send(logger.ELEVATOR, "ESH: ERROR (#2)!! Height = %f \n", targetHeightMm);
      return;
    }
...
bool ElevatorSub::isElevatorAtTarget()
{
  ...
  logger.send(logger.ELEVATOR, "IEAT: is Elevator at target %f \n", elevatorTargetHeightMm);
  ...
  {
    logger.send(logger.ELEVATOR, "IEAT: True is Elevator at target %f current: %f Velocity: %f \n", elevatorTargetHeightMm, getElevatorHeight(), getElevatorVelocity());
    return true;
  }
  else
  {
    logger.send(logger.ELEVATOR, "IEAT: False is Elevator at target %f \n", elevatorTargetHeightMm);
    return false;
  }
}
